...
- 2026-05-01 — EntityCard refactored to strict Composable → ViewModel → UseCase + UIModel boundary per project-wide architecture rule (see `_bmad/outputs/architecture.md` § Compose UI Boundary). Added `EntityCardViewModel` (Koin parameterized factory keyed by `entityId`), `EntityCardUiModels.kt` (sealed `EntityCardUiState` with `Toggle`/`Stepper`/`Trigger`/`Media`/`Unknown`/`ReadOnly` variants + `EntityCardIntent`), `EntityCardViewModelTest.kt`. Composable + previews now consume `EntityCardUiState` only — no `HaEntity` imports in UI. Body API: `EntityCardBody(state, onIntent, size, modifier)`.
